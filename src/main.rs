...
fn fft(xs: Vec<f64>) -> Vec<Complex<f64>> {
    ...
    let factor_mul: Complex<f64> = Complex::new(0.0, -2.0 * PI);

    let mut c_p1: Vec<Complex<f64>> = Vec::new();
    let mut c_p2: Vec<Complex<f64>> = Vec::new();

    for i in (0..(n/2)) {
        c_p1.push(
            x_even_res[i] + 
            (factor_mul * Complex::new(i as f64, 0.0) / Complex::new(n as f64, 0.0)).exp() * 
            x_odd_res[i]
        );
        c_p2.push(
            x_even_res[i] + 
            (factor_mul * Complex::new((i + (n / 2)) as f64, 0.0) / Complex::new(n as f64, 0.0)).exp() *
            x_odd_res[i]
        );
    }    
